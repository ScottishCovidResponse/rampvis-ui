...
export class SensitivitySmallMultiple {
  ...
  constructor(options) {
    ...
    let meanLineColor = options.meanLineColor
      ? options.meanLineColor
      : "#E00015";
    let sampleLineColor = options.sampleLineColor
      ? options.sampleLineColor
      : "#A6CCD8";
    let sampleDotColor = options.sampleDotColor
      ? options.sampleDotColor
      : "#4171D9";
    let containerChart = options.containerChart
    ...
    const CHART_WIDTH =
      document.getElementById(containerChart).offsetWidth * widthfactor;
    ...
    CHART_HEIGHT < CHART_WIDTH / (n_rows - 1)
      ? (CHART_HEIGHT = CHART_HEIGHT)
      : (CHART_HEIGHT = (CHART_WIDTH / (n_rows - 1)) * n_columns);
        ...
        return d3
          ...
          .x((d) => {
            return Math.min(
              ...
              Math.max(
                x_temp(
                  d3.select(this.parentNode.parentNode.parentNode).datum(),
                )(d.x),
                0,
              ),
            );
    ...
    svg
      ...
      .selectAll("dot")
      .data((d) => {
        return d.values[0].dataPoints;
      })
      .enter()
      .append("circle")
      .attr("cx", function (d) {
        return x_temp(d3.select(this.parentNode.parentNode.parentNode).datum())(
          d.x,
        );
      })
      .attr("cy", function (d) {
        return y_temp(d3.select(this.parentNode.parentNode.parentNode).datum())(
          d.y,
        );
      })
      .attr("r", 2.5)
    ...
    svg
      ...
      .attr("x", -120)
      .text(function (d) {
        return d.values[0].scalarFeature + " of " + d.values[0].quantityName;
      })
