 ...
 */
export function mirroredStackedBarChart() {
  /**
   * Visual configs.
   */
  ...
  function update() {
    ...
    xScale.domain(data.top.map(label)).range([0, width]);
    yScaleTop
     ...
     * Draw.
     */
    const topSectionLabel = data.top[0][splitAttribute] || "Top";
    itemContainer
      .selectAll("g.top")
      .data(seriesTop)
      .join("g")
    ...
    const bottomSectionLabel = data.bottom[0][splitAttribute] || "Bottom";
    itemContainer
      .selectAll("g.bottom")
      .data(seriesBottom)
      .join("g")
