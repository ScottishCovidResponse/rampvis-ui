...
export class UncertaintySampleAndMean {
  ...
  constructor(options) {
    ...
    // Add X axis
    const x = d3
      .scaleLinear() //THE PLUS SIGN IS SUPER IMPORTANT IT MAKES THE VALUE BE TREATED AS A NUMBER!
    ...
    const y = d3
      ...
      .domain([
        0,
        Math.max(
          d3.max(dataAll, (d) => +d[quantityName]),
          yMax,
        ),
      ])
      .range([height, 0]);
    ...
    function resize() {
      let h =
        (window.innerHeight - Common.MAIN_CONTENT_GAP - gap) * heightfactor;
