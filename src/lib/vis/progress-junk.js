...
class ProgressJunk extends React.Component {
    constructor(props) {
        ...
        this.progressValue = parseInt((this.positive / this.domainLength) * this.midPoint);
        this.negativeValue = parseInt((this.negative / this.domainLength) * this.midPoint);
        this.meanValue = parseInt((this.mean / this.domainLength) * this.totalWidth);

        this.positiveScale = d3.scaleLinear().domain([0, this.domainLength]).range(["#00FF7F", "#2E8B57"])
        this.negativeScale = d3.scaleLinear().domain([0, this.domainLength]).range(["#FF66B2", "#660033"])
