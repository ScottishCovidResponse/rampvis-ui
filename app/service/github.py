...
class GitHub(object):
    ...
    def get_profile(self, access_token):
        ...
        r = self.raw_request('GET', BASE_URL + '/user?access_token={}'.format(access_token))
        if not r.ok:
            raise Exception("Could not load profile data from Github API")
        profile = json.loads(r.text or r.content)

        r = self.raw_request('GET', BASE_URL + '/user/emails?access_token={}'.format(access_token))
        if not r.ok:
...
def initialize_github_login(app):
    global github
    github = GitHub(app)
    return github
...
def get_profile(access_token):
    ...
    r = requests.get(BASE_URL + '/user?access_token={}'.format(access_token))
    if not r.ok:
        raise Exception("Could not load profile data from Github API")
    profile = json.loads(r.text or r.content)

    r = requests.get(BASE_URL + '/user/emails?access_token={}'.format(access_token))
    if not r.ok:
        raise Exception(_("Could not load emails data from from Github API"))
    emails = json.loads(r.text or r.content)
